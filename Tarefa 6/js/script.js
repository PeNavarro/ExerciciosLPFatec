...
$('#btnEnviar')[0].addEventListener('click', () => {
      ...
      if($('#name')[0].validity.valid && $('#tel')[0].value.length == 15 && $('#email')[0].validity.valid){
            ...
            if(comidas.length){
                  for (let i = 0; i < comidas.length; i++) {
                        if(comida[i].quantity > 0){
                              ...
                              pedido = pedido+`<li>${comida[i].type}: ${comida[i].name} - Preço unitário: R$ ${precoExibido} - Quantidade: ${comida[i].quantity} - Total: R$ ${totalExibido}.</li>`
                        }
                  }
            }
            ...
            if(pedido == ""){
                  ...
                  $('.pedidoCheio p:last b')[0].innerHTML = `Preço final R$ ${precoFinal}`
            }
